...
**Architecture:** Admin uploader PDF'er i Mæt → Storage `tilbudsaviser/inbox/` + en `tilbud_import_job`-række → edge-funktionen `start-tilbud-import` (admin-JWT-gate) affyrer en GitHub `repository_dispatch` → en GitHub Action kører det *eksisterende* Node-udtræk (renderer sider, kalder `importer-tilbud`, skriver `tilbud`) og opdaterer job-status → appen viser status og henter `tilbud` live.
