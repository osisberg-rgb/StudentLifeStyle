...
- `importer-tilbud` — one weekly-deal page (image) → `{ varer: [{ navn, pris, soeg }] }` via gpt-4o-mini vision, `soeg` filtered to a fixed vocab. Invoked by `scripts/opdater-tilbud.ps1`/`.mjs` (the weekly PDF→`tilbud`-table pipeline), **not** the app.
